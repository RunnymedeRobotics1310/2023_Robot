...
public class RunnymedeUtils {
    ...
    public static double calculateFastestSpeed(final double distanceToTravel, final double distanceTravelled,
        ...
        if (startSpeed < 0) throw new IllegalArgumentException("Start speed "+startSpeed+" must not be negative");
        if (endSpeed < 0) throw new IllegalArgumentException("End speed "+endSpeed+" must not be negative");
        if (maxSpeed < 0) throw new IllegalArgumentException("Max speed "+maxSpeed+" must not be negative");
